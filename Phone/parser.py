		...
		while(sent == False):
			...
			try:
				...
				if (color >= 0):	
					#send color
					pprint.pprint(color)
					pprint.pprint((color << 1) & 0xFE)
					pprint.pprint((color >> 6) & 0xFE)
					...
					if(droid.bluetoothRead().result is None):
							raise Exception
				...
				if (len(outputText) > 0):	
					...
					#send frame1 - sending characters one at a time is more reliable	
					...
					#send frame2 - sending characters one at a time is more reliable	
